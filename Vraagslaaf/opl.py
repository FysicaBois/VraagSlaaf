...
class Oplossing(commands.Cog):
    ...
    The PyMuPDF library is very powerful but also poorly documented and fairly low-level; which makes for a not so enjoyable coding experience.

    """

    def __init__(self):
        self.doc = fitz.open("Vraagslaaf/test.pdf")
        ...
        self.scale_factor = fitz.matrix=fitz.Matrix(5, 5)
        self.croptop = 50
        self.cropbottom = 65

    @commands.Cog.listener()
    async def on_ready(_):
      Log.info("Oplossing Cog is up!")
        ...
        for p in doc.pages(int(self.chapters[int(splits[0])][0]), int(self.chapters[int(splits[0])][1])):
            ...
            if oplG:
                ...
                if oefnr in p.getText():
                    ...
                    # todo this should be replaced by the oplossingen Cog
    ...
    async def opl(self, ctx,  *, argv):
        try:
            ...
            await ctx.send(file=discord.File("SPOILER-finished.png"))
        except:
            await ctx.send("Er is iets foutgegaan chief, sorry!")

    @commands.command(name="opl")
    async def opl(self, ctx, *, arg):
        oplG = False
        try:
            splits = arg.split('.')
            todopage = None
            for p in self.doc.pages(int(self.chapters[int(splits[0])][0]), int(self.chapters[int(splits[0])][1])):
                if "Solutions to Problems" in p.getText():
                    oplG = True
                if oplG:
                    if (str(splits[1] + ". ")) in p.getText():
                        todopage = p
                        break

            pix = todopage.getPixmap(matrix=fitz.Matrix(8, 8))  # zoom eight times
            pix.writeImage("Vraagslaaf/SPOILER_temp.png")
            await ctx.send(file=discord.File("Vraagslaaf/SPOILER_temp.png"))
        except:
            await ctx.send("Ik heb je commando niet begrepen chief :(")

    @cog_ext.cog_slash(name="opl",
                       description="Vraagt een Oplossing uit Giancoli 1 of 2 op.",
                       ...
                       options=[
                           create_option(
                               name="hoofdstuk",
                               description="Nummer van het hoofdstuk uit Giancoli. Voorbeeld: 2",
                               option_type=4,
                               required=True),
                           create_option(
                               name="oefening",
                               description="Nummer van de oefening. Voorbeeld: 69.",
                               option_type=4,
                               required=True)
                       ])
    async def _opl(self, ctx: InteractionContext, hoofdstuk: int, oefening: int):
        await ctx.defer()
        oplG = False
        try:
            todopage = None
            for p in self.doc.pages(int(self.chapters[hoofdstuk][0]), int(self.chapters[hoofdstuk][1])):
                if "Solutions to Problems" in p.getText():
                    oplG = True
                if oplG:
                    if (str(oefening) + ". ") in p.getText():
                        todopage = p
                        break
