define(['jquery', 'rngModule', 'audioModule'], function($, rngModule, audioModule) {
  ...
  var reelModuleIsSpinning = function(){
	...
	var reelModuleSpin = function(data) {
		if(!reelSpinning[0] && !reelSpinning[1] && !reelSpinning[2]) {
			var reelNumbers = data,
      ...
      console.log(reelNumbers)
			...
			reelContainer.children('div').each(function(index) {
                ...
                /*
                     $.ajax({
                  type: "POST",
                  url: "https://vraagslaaf.aap007freak.repl.co/result",
                  data: {reelNumbers},
                });
                */
               
                var targetContainer = document.getElementById("spinning-user");
                targetContainer.innerHTML = "type slaaf rol <hoeveelheid> in Discord om te rollen";
    ...
    reelmoduleisspinning: reelModuleIsSpinning
	};
});